...
To verify that it works, consider calculating a few multiples of 3 by hand in a notebook (for example, 102, 105, 108, etc.), then making sure they're being replaced with "Loopy" in your output.
...
/*
for (let i = 100; i <= 200; i++) {
  if ((i % 4 === 0) && (i % 4 === 0)) { console.log("LoopyLighthouse") }
...
// The below is another way of writing the same thing. The Compass Prep site wanted it to look this way (using modulo 12 instead of modulo 3 and modulo 4 both being true)
for (let i = 100; i <= 200; i++) {
